...
namespace MasterServerToolkit.Bridges.FishNetworking
{
    public class RoomClientManager : RoomClient<RoomClientManager>
    {
        ...
        protected override void Start()
        {
            ...
            {
                logger.Warn($"Before using {typeof(RoomNetworkManager).Name} add it to scene");
            }
        }
        ...
        private void ClientManager_OnClientConnectionState(ClientConnectionStateArgs state)
        {
            ...
            {
                if (Mst.Client.Rooms.ReceivedAccess != null)
                {
        ...
        public override void StartConnection(RoomAccessPacket access)
        {
            if (isChangingZone)
                StartDisconnection();
        ...
        /// </summary>
        public override void StartDisconnection()
        {
            if (isChangingZone)
        ...
        public void AutoConnectGame(int gameId)
        {
            MstProperties props = new();
            props.Add(Mst.Args.Names.IsBotClient, Mst.Args.IsBotClient);
            Mst.Client.Rooms.GetAccess(gameId, (access, getAccessError) =>
            {
                logger.Debug($"Connected to game {gameId}");
