...
namespace Framework
{
    ...
    public class StateMachine<TLabel>
    {
        ...
        public void AddState(TLabel label, IState stateMachine)
        {
            stateMachine.Init();
        ...
        /// <param name="onUpdate">The action performed when the state machine is updated in the given state.</param>
        /// <param name="onStop">The action performed when the state machine is left.</param>
        public void AddDelegate(TLabel label, Action onStart, Action onUpdate, Action onStop)
        {
            if (stateDictionary.ContainsKey(label))
            {
                stateDictionary[label].onStart += onStart;
                stateDictionary[label].onStop += onStop;
                stateDictionary[label].onUpdate += onUpdate;
            }
            else
            {
                AddState(label, onStart, onUpdate, onStop);
            }
        }
        /// <summary>
        /// 
        ...
        /// <param name="onUpdate">The action performed when the state machine is updated in the given state.</param>
        /// <param name="onStop">The action performed when the state machine is left.</param>
        public void RemoveDelegate(TLabel label, Action onStart, Action onUpdate, Action onStop)
        {
            if (stateDictionary.ContainsKey(label))
            {
                stateDictionary[label].onStart -= onStart;
        ...
        {
            if (currentState == null || !currentState.label.Equals(newState))
            {
                ...
                OnStateChanged?.Invoke(currentState.label);
            }
        }
        ...
        public event Callback<TLabel> OnStateChanged;
