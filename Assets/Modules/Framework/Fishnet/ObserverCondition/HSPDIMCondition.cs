    ...
    public class HSPDIMCondition : ObserverCondition
    {
        ...
        public override bool ConditionMet(NetworkConnection connection, bool currentlyAdded, out bool notProcessed)
        {
            ...
            foreach (NetworkObject nob in connection.Objects)
            {
                ...
                if (!HSPDIM.Instance.isRunning) break;
                var e2 = nob.GetComponent<HSPDIMEntity>();
                if (e1.Id<e2.Id)
                {
                    if (HSPDIM.Instance.Result[0].Contains(new(e1.Id, e2.Id, 0)) && HSPDIM.Instance.Result[1].Contains(new(e1.Id, e2.Id, 0)))
                        return true;
                    //if (HSPDIM.Instance.HSPDIMEntities[e1.Id].UpRange.intersectionId.Contains(e2.Id)) return true;
                }
                else
                {
                    if (HSPDIM.Instance.Result[0].Contains(new(e2.Id, e1.Id, 1)) && HSPDIM.Instance.Result[1].Contains(new(e2.Id, e1.Id, 1)))
                        return true;
