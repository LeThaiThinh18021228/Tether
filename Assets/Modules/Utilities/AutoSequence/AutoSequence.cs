...
namespace Utilities
{
    public class AutoSequence : CacheMonoBehaviour
    {
        ...
        void OnDrawGizmos()
        {
            ...
            Vector3 lastPos = CacheTransform.localPosition;
            ...
            for (int i = 0; i < _steps.Count; i++)
            {
                ...
                if (deltaPos != Vector3.zero)
                {
                    ...
                    {
                        GizmosHelper.DrawLine(lastPos, lastPos + deltaPos, Color.yellow);
                        GizmosHelper.DrawWireSphere(lastPos + deltaPos, 0.2f, Color.red);
                    }
                    else
                    {
                        GizmosHelper.DrawLine(CacheTransform.parent.TransformPoint(lastPos), CacheTransform.parent.TransformPoint(lastPos + deltaPos), Color.yellow);
                        GizmosHelper.DrawWireSphere(CacheTransform.parent.TransformPoint(lastPos + deltaPos), 0.1f, Color.red);
                    }
